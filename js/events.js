...
function generateCalendar(month, year) {
    ...
    for (let i = firstDay - 1; i >= 0; i--) {
        ...
        currentRow += `<div class="calendar-day" style="flex: 1; min-width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; color: #d1d5db; cursor: default;">${prevDay}</div>`;
        dayCounter++;
    }
    ...
    // Next month days to fill remaining squares (grayed out)
    let nextDay = 1;
    while (dayCounter < 42) { // 6 weeks × 7 days = 42 squares
        currentRow += `<div class="calendar-day" style="flex: 1; min-width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; color: #d1d5db; cursor: default;">${nextDay}</div>`;
        dayCounter++;
        nextDay++;
